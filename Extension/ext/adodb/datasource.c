...
ZEPHIR_INIT_CLASS(ADOdb_DataSource) {
	...
	zend_declare_class_constant_string(adodb_datasource_ce, SL("DSN_PARSE_REGEXP"), "|^(?P<type>\\w+)([\\(](?P<dbsyntax>\\w+)[\\)])?(::?[/]{0,2}/(((?P<username>\\w+)(:(?P<password>\\w+))?)@)?(((?P<protocol>\\w+)\\+)?(?P<hostname>\\w+)(:(?P<port>\\d+))?)?(\\/?(?P<database>[^\\s\\?]*)?)?(\\?(?P<options>\\S+))?)?$|i" TSRMLS_CC);
...
PHP_METHOD(ADOdb_DataSource, parseDSN) {
	...
	zval *dsn_param = NULL, *p, *options = NULL, *_0, *pos, _1 = zval_used_for_init, *_2 = NULL, _3, *_4, *_5 = NULL, *output, *_6;
	zval *dsn = NULL, *_7;
	...
	array_init(p);
	ZEPHIR_INIT_VAR(_0);
	zephir_substr(_0, dsn, 0 , 6 );
	if (ZEPHIR_IS_STRING(_0, "sqlite")) {
		ZEPHIR_SINIT_VAR(_1);
		ZVAL_STRING(&_1, ":", 0);
		ZEPHIR_INIT_VAR(pos);
		zephir_fast_strpos(pos, dsn, &_1, 0 );
		ZEPHIR_INIT_VAR(_2);
		zephir_substr(_2, dsn, 0 , zephir_get_intval(pos) );
		zephir_array_update_string(&p, SL("type"), &_2, PH_COPY | PH_SEPARATE);
		ZEPHIR_SINIT_VAR(_3);
		ZVAL_LONG(&_3, (zephir_get_numberval(pos) + 1));
		ZEPHIR_INIT_VAR(_4);
		zephir_substr(_4, dsn, zephir_get_intval(&_3) , 0 );
		zephir_array_update_string(&p, SL("database"), &_4, PH_COPY | PH_SEPARATE);
		array_init_size(return_value, 3);
		ZEPHIR_OBS_VAR(_5);
		zephir_array_fetch_string(&_5, p, SL("type"), PH_NOISY TSRMLS_CC);
		zephir_array_update_string(&return_value, SL("type"), &_5, PH_COPY | PH_SEPARATE);
		ZEPHIR_OBS_NVAR(_5);
		zephir_array_fetch_string(&_5, p, SL("database"), PH_NOISY TSRMLS_CC);
		zephir_array_update_string(&return_value, SL("database"), &_5, PH_COPY | PH_SEPARATE);
		RETURN_MM();
	} else {
		ZEPHIR_INIT_NVAR(_2);
		ZEPHIR_SINIT_NVAR(_1);
		ZVAL_STRING(&_1, "|^(?P<type>\\w+)([\\(](?P<dbsyntax>\\w+)[\\)])?(::?[/]{0,2}/(((?P<username>\\w+)(:(?P<password>\\w+))?)@)?(((?P<protocol>\\w+)\\+)?(?P<hostname>\\w+)(:(?P<port>\\d+))?)?(\\/?(?P<database>[^\\s\\?]*)?)?(\\?(?P<options>\\S+))?)?$|i", 0);
		zephir_preg_match(_2, &(_2), &_1, dsn, p, 0, 0 , 0  TSRMLS_CC);
		if (zephir_is_true(_2)) {
			...
			zephir_array_update_string(&return_value, SL("type"), &_5, PH_COPY | PH_SEPARATE);
			ZEPHIR_OBS_NVAR(_5);
			zephir_array_fetch_string(&_5, p, SL("dbsyntax"), PH_NOISY TSRMLS_CC);
			zephir_array_update_string(&return_value, SL("dbsyntax"), &_5, PH_COPY | PH_SEPARATE);
			ZEPHIR_OBS_NVAR(_5);
			zephir_array_fetch_string(&_5, p, SL("protocol"), PH_NOISY TSRMLS_CC);
			zephir_array_update_string(&return_value, SL("protocol"), &_5, PH_COPY | PH_SEPARATE);
			ZEPHIR_OBS_NVAR(_5);
			zephir_array_fetch_string(&_5, p, SL("hostname"), PH_NOISY TSRMLS_CC);
			zephir_array_update_string(&return_value, SL("hostname"), &_5, PH_COPY | PH_SEPARATE);
			ZEPHIR_OBS_NVAR(_5);
			zephir_array_fetch_string(&_5, p, SL("port"), PH_NOISY TSRMLS_CC);
			zephir_array_update_string(&return_value, SL("port"), &_5, PH_COPY | PH_SEPARATE);
			ZEPHIR_OBS_NVAR(_5);
			zephir_array_fetch_string(&_5, p, SL("database"), PH_NOISY TSRMLS_CC);
			zephir_array_update_string(&return_value, SL("database"), &_5, PH_COPY | PH_SEPARATE);
			ZEPHIR_OBS_NVAR(_5);
			zephir_array_fetch_string(&_5, p, SL("username"), PH_NOISY TSRMLS_CC);
			zephir_array_update_string(&return_value, SL("username"), &_5, PH_COPY | PH_SEPARATE);
			ZEPHIR_OBS_NVAR(_5);
			zephir_array_fetch_string(&_5, p, SL("password"), PH_NOISY TSRMLS_CC);
			zephir_array_update_string(&return_value, SL("password"), &_5, PH_COPY | PH_SEPARATE);
			...
			ZEPHIR_INIT_VAR(_7);
			ZEPHIR_CONCAT_SV(_7, "Invalid DSN: ", dsn);
...
PHP_METHOD(ADOdb_DataSource, setDsn) {
	...
	for (
	  ...
	  ; zephir_hash_move_forward_ex(_2, &_1)
		...
		zephir_update_property_zval_zval(this_ptr, name, val TSRMLS_CC);
	}
	RETURN_THIS();
